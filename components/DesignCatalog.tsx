...
export const DesignCatalog: React.FC<DesignCatalogProps> = ({ catalog }) => {
  ...
  }, [expandedItem, elementDetails, addToast]);
  ...
  const renderItemList = (
    items: { name: string; species?: string; description?: string }[],
      ...
      {items.map((item) => {
        ...
        const content = (
          <div>
            ...
            <p className="text-xs text-slate-500">{item.species || item.description}</p>
          </div>
        );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {hasFeatures && (
          <section>
            ...
            {renderItemList(catalog.features, false)}
          </section>
        )}
      </div>
    </div>
